    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `RECORDS` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT NOT NULL, `path` TEXT NOT NULL, `date` TEXT NOT NULL)');
...
class _$RecordDao extends RecordDao {
  ...
  Future<void> deleteRecordById(int id) async {
    await _queryAdapter
        .queryNoReturn('DELETE FROM RECORDS WHERE id = ?1', arguments: [id]);
  }

  @override
  Future<int> insertRecord(Record record) {
    return _recordInsertionAdapter.insertAndReturnId(
        record, OnConflictStrategy.abort);
  }

  @override
  Future<int> updateRecord(Record record) {
    return _recordUpdateAdapter.updateAndReturnChangedRows(
        record, OnConflictStrategy.abort);
  }
}

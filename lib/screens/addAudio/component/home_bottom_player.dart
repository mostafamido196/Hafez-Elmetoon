...
import 'package:hafez_elmetoon/res/app_images.dart';
...
class BottomPlayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<PlayerBloc, PlayerState>(
      ...
      builder: (context, state) {
        ...
        print('bottom player state : $state');
  ...
  Widget _row(BuildContext context, PlayerState state) {
    // return Container();
    return Row(
      children: [
        const CircleAvatar(
          ...
          child: Column(
            ...
            children: [
              BlocBuilder<PlayerBloc, PlayerState>(
                builder: (context, state) {
                  return Text(
                    state.file == null ? '' : state.file!.name.toString(),
              ...
              Text(
                state.file == null ? '' : state.file!.length.toString(),
                style: const TextStyle(
                    color: Colors.grey,
                    fontSize: 11,
                    fontWeight: FontWeight.bold),
            ...
            GestureDetector(
              onTap: () {
                ...
                bloc.add(PlayPauseEvent(
                    isPlaying: !bloc.state.isPlaying, file: bloc.state.file!));
              },
              child: CircleAvatar(
                radius: 20,
